...
} from "./interfaces/IRecommendationEngine";
import {
...
export class RecommendationEngine implements IRecommendationEngine {
  ...
  static fromRecommendationEngineState(
    state: IRecommendationEngineState,
    exercises: IExerciseData
  ): IRecommendationEngine {
    ...
    const helpfulnessOracle = Oracle.fromOracleState(
      state.helpfulnessOracleState
    );
  ...
  _filteredExerciseIds(context: IContext): string[] {
    ...
    for (const exerciseId in this.exercises) {
      ...
      if (
        ...
        (context?.stressed ?? 0 >= 4) &&
        exercise.Features.relax == 0 &&
        exercise.Features.breathing == 0
      ) {
        continue;
      }
      if ((context?.sad ?? 0 >= 4) && exercise.Features.mindfulness == 1) {
        continue;
      }
  ...
  makeRecommendation(context: IContext) {
    ...
    for (let i = 0; i < filteredExerciseIds.length; i++) {
      ...
      const exercise = this.exercises[exerciseId];
      const clickScore = this.clickOracle.predict(
        processedContext,
        exercise.Features,
        exercise.ExerciseId
      );
      const likingScore = this.likingOracle.predict(
        processedContext,
        exercise.Features,
        exercise.ExerciseId
      );
      const helpfulnessScore = this.helpfulnessOracle.predict(
        processedContext,
        exercise.Features,
        exercise.ExerciseId
      );
    ...
    let SoftmaxDenominator = scoredExercises.reduce(
      (a, b) => a + b.probability,
      0
    );
  ...
  private _generateDirectChoiceTrainingData(
    exerciseId: string
  ): IExerciseTrainingData[] {
    const exercise = this.exercises[exerciseId];
    if (!exercise) {
      throw new Error(
        `Failed to generate training data for exercise with id ${exerciseId}.`
      );
    }
    const input = {
      exerciseId: exerciseId,
  ...
  private _generateClickTrainingData(
    recommendation: IRecommendation,
    selectedExerciseId: string | undefined = undefined
  ): IExerciseTrainingData[] {
    let trainingData: IExerciseTrainingData[] = [];
    for (
      let index = 0;
      index < recommendation.recommendedExercises.length;
      index++
    ) {
      ...
      const clicked =
        recommendedExercise.ExerciseId === selectedExerciseId ? 1 : 0;
      const liking = undefined;
      const helpfulness = undefined;
      const probability =
        recommendation.recommendedExercises[index].probability;
  ...
  private _generateEvaluationTrainingData(
    processedContext: IProcessedContext,
    exerciseId: string,
    ...
    if (!evaluatedExercise) {
      throw new Error(
        `Failed to generate training data for evaluated exercise at index ${exerciseId}.`
      );
    }
    ...
    const probability = 1;
    const trainingData: IExerciseTrainingData = {
      input,
      clicked,
      liking,
      helpfulness,
  ...
  onChooseRecommendedExercise(
    ...
    return new Promise((resolve, reject) => {
      try {
        const trainingData = this._generateClickTrainingData(
          recommendation,
          exerciseId
        );
        this.clickOracle.fitMany(trainingData);
    ...
    return new Promise((resolve, reject) => {
      try {
        const processedContext =
          possibleRecommendationContext ??
          processContext(evaluationTimeContext);
        const trainingData = this._generateEvaluationTrainingData(
          processedContext,
          exerciseId,
          evaluation
        );
...
export class DemoRecommendationEngine
  extends RecommendationEngine
  implements IDemoRecommendationEngine
{
  scoreAllExercises(context: IContext): IScoredExercise[] {
    ...
    const sortedExercises = scoredExercises.sort(
  ...
  getRecommendedExercises(recommendation: IRecommendation): IExercise[] {
    return recommendation.recommendedExercises.map(
      (ex) => this.exercises[ex.exerciseId]
    );
  }
}
